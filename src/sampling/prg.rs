//! Primitive §1.1 — SHAKE-256 counter-mode deterministic PRG.
//!
...
//! every seed length the spec calls for (the reference uses ≤ 32-byte seeds).
//!
...
impl Shake256Prg {
    ...
    /// is the same procedure as Python's `int.from_bytes(..., "little") & mask`,
    ...
    /// matching the reference's early-return path.
    ///
    /// # Panics
    ///
    /// Panics if `bound == 0`.
    ...
    pub fn uniform_below(&mut self, bound: u64) -> u64 {
        ...
        // For `bound > 1`, `bits` lies in `[1, 64]`. The Python reference uses
        // `bound.bit_length()`, which equals `64 - bound.leading_zeros()` here.
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Byte-stream parity with the Python reference.
    //
    // The constants below were generated by running:
    //   import hashlib, struct
    //   def stream(seed, n):
    //       out, ctr = b"", 0
    //       while len(out) < n:
    //           h = hashlib.shake_256(seed + struct.pack("<Q", ctr))
    //           out += h.digest(136); ctr += 1
    //       return out[:n].hex()
    // against the Python reference's `DeterministicSampler` at counter zero.
    ...
    //
    // The expected sequences below were generated by running Python's
    // `DeterministicSampler(seed).randbelow(bound)` repeatedly.
    // -----------------------------------------------------------------------

    /// First 8 outputs of `DeterministicSampler(b"test").randbelow(3)`.
    const TEST_SEED_RANDBELOW_3_FIRST_8: [u64; 8] = [1, 2, 1, 2, 2, 0, 2, 2];

    /// First 8 outputs of `DeterministicSampler(b"test").randbelow(256)`.
    const TEST_SEED_RANDBELOW_256_FIRST_8: [u64; 8] = [193, 237, 190, 43, 42, 176, 229, 190];

    /// First 4 outputs of `DeterministicSampler(b"test").randbelow(8380417)`
    /// (paper VIA-C $q_3$, ≈ $2^{23}$).
    const TEST_SEED_RANDBELOW_Q3_FIRST_4: [u64; 4] = [7_199_425, 3_980_854, 662_059, 8_268_469];

    /// First 4 outputs of `DeterministicSampler(b"test").randbelow(2**32)`.
    ...
    /// First 4 outputs of `DeterministicSampler(b"test").randbelow(2**32 + 1)`.
    ...
    fn uniform_below_paper_q1_factor_parity() {
        // The smaller of the paper's VIA-C q1 RNS primes: 137_438_822_401
