//! Discrete Gaussian sampler over $\mathbb{Z}$ via the
...
//! $$
//!
//! returning $\lfloor z \cdot \sigma \rceil$ (round-half-to-even).
//!
...
//! asymmetry the `(0, 1]` distribution requires.
...
/// only. The asymmetry with [`TWO_POW_32_F64`] is intentional.
...
/// (banker's rounding), implemented via [`libm::rint`]. This is the
/// standard round-half-to-even for IEEE 754 doubles.
///
/// # Edge cases
///
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Stage D — byte-exact parity vectors. Generated by running the
    // discrete-Gaussian sampler at the given seed, length, and σ.
    // -----------------------------------------------------------------------
    ...
    fn tail_within_three_sigma() {
        // Shape test for the discrete Gaussian.
