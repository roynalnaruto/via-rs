//! The [`crate::lwe_to_rlwe_cascade!`] macro and the toy-parameter
//! cascade instantiations it produces.
//!
...
//! const-generics, so it cannot be a runtime loop. Instead
//! [`crate::lwe_to_rlwe_cascade!`] emits, per concrete `n`:
//!
...
//!
//! Only the toy sizes are instantiated here (`n = 8` for the KAT, `n = 4` as a
//! smoke test, and `n = 8` on the RNS backend); the protocol layer invokes the
//! macro for the paper `n₁`, and VIA-B reuses the same step machinery.
...
macro_rules! lwe_to_rlwe_cascade {
    ...
    (
        ...
        #[doc = concat!(
            ...
            stringify!($n),
            " LWE→RLWE conversion: one fixed-size array of RLev step keys per ",
        ...
        #[doc = concat!(
            "Generate the degree-", stringify!($n),
        ...
        #[cfg(feature = "alloc")]
        #[doc = concat!(
            "Heap-build the degree-", stringify!($n),
...
// ---------------------------------------------------------------------------
// SPIKE — depth-18 noise GO/NO-GO + boxed-builder peak-stack check.
//
