...
//! product consumes to perform homomorphic multiplication
...
//! $$
//!
//! The two halves can use **different gadget parameters** — distinct
//! `(L, B)` for many call sites. The Rust API
...
impl<const N: usize, R: RingPoly<N>> SecretKey<N, R> {
    /// Encrypt `message` as an RGSW: pair of RLevs
    /// `(RLev_S(-S · M), RLev_S(M))`.
    ///
    /// Different gadget bases per half are supported (`base_neg_s_m`
    /// for the `-S · M` half, `base_m` for the `M` half), as DMux/CMux
    /// halves may use distinct `(L, B)`.
    /// Pass the same value for both to recover the
    /// symmetric behaviour.
    ///
    /// # PRG consumption order
    ///
    /// The `-S · M` half is encrypted **first** (consuming `L1`
    /// per-level RLWE draws), then the `M` half (`L2` draws).
    /// Flipping the order would silently break
    /// parity for every RGSW test vector.
    ///
    /// # Constant-time
    ///
    ...
    /// key material only enter the picture in secret-key
    /// rekeying.
    pub fn encrypt_rgsw<const L1: usize, const L2: usize>(
        ...
        // The `-S·M` half first (fixes the consumption ordering).
...
impl<const N: usize, R: RingPoly<N>, const L1: usize, const L2: usize>
    RGSWCiphertext<N, R, L1, L2>
{
    /// External product: $\mathrm{RGSW}_S(M_1) \boxtimes
    ...
    /// same value for both to recover the symmetric
    ...
    /// the decryption budget at the standard parameters.
        ...
        R: RingPolyEval<N>,
    {
        ...
        ct1 + ct2 // `Add for RLWECiphertext`.
    }
}
...
mod tests {
    ...
    /// External-product imports for tests.
    ...
    /// Realistic single-prime: VIA-C `q₂`, p=256, CMux-sel gadget
    ...
    /// Realistic RNS: VIA-C `q₁` (Q ≈ 2⁷⁵), p=2, DMux-ctrl gadget
