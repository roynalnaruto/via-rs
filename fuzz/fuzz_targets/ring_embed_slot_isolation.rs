...
//! at slot `j`, projecting at any *other* slot yields the zero
//! polynomial — slots are disjoint.
...
fn check<M: Modulus>(m: M, values: [u64; N_SMALL], slot: usize) {
    ...
    let zero: Poly<N_SMALL, M, Coefficient> = Poly::zero(m);
    let slot = slot % D;
    ...
    for jp in 0..D {
        ...
        let back: Poly<N_SMALL, M, Coefficient> = big.project_at::<N_SMALL>(jp);
